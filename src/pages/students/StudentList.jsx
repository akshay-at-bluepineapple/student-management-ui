...
const StudentList = () => {
  ...
  const { studentList, isLoading, error } = useSelector((state) => state.student);
  ...
  return (
    <section className="p-6 mx-auto mt-8 font-mono">
      <h2 className="mb-6 text-2xl font-bold text-center">Student List</h2>
              ...
              <tbody className="bg-white">
                {studentList.length === 0 ? (
                  ...
                  studentList.map((student) => (
                    ...
                    >
                      <td className="px-4 py-3 border">{student.registrationNo}</td>
                      <td className="px-4 py-3 border">{student.fullname}</td>
                      <td className="px-4 py-3 border">{student.age}</td>
                      <td className="px-4 py-3 border">{student.class_enrolled}</td>
                      <td className="px-4 py-3 border">{student.phoneNumber}</td>
                      <td className="px-4 py-3 border">{student.pen_fees === "0.00" ? "Paid" : "Pending"}</td>
                      <td className="px-4 py-3 border">{student.total_fees_paid }</td>
                      <td className="px-4 py-3 border">{student.pen_fees }</td>
                      <td className="px-4 py-3 border">{student.total_fees }</td>
                      <td className="px-4 py-3 border">{student.academic_year}</td>
                      <td className="px-4 py-3 border">{student.enrollment_date}</td>
                      <td className="px-4 py-3 border">
                        {student.is_active ? (
                          <span className="px-2 py-1 text-green-700 bg-green-100 rounded">Active</span>
                        ) : (
                          <span className="px-2 py-1 text-red-700 bg-red-100 rounded">Inactive</span>
                        )}
                      </td>
                    </tr>
                  ))
